...
curl -X PUT http://${HOST}/buckets/ssmix/props -H 'Content-type:application/json' -d '{"props":{"search_index":"md_index"}}'
curl -X PUT http://${HOST}/buckets/rezept/props -H 'Content-type:application/json' -d '{"props":{"search_index":"md_index"}}'
